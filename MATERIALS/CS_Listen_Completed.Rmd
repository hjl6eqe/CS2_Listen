---
title: "Recession Prediction Case Study"
author: "Harrison Listen (hjl6eqe)"
date: "5/1/2023"
output: html_document
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```
# Install the Necessary Packages
```{r, message=FALSE}
#install.packages("forecast")
library(dplyr)
library(readr)
library(readxl)
library(forecast)
#install.packages("vars")
library(vars)
#install.packages("ggforitfy")
#library(ggfortify )
library(Metrics)
library(tseries)
```
# Read in the Data
```{r}
econdata <- read.csv('/Users/harrisonlisten/Desktop/CS_Cleaned_Data.csv', row.names = "Year")
```
# Partition the Data and Train the VAR Model
```{r, warning=FALSE}
data <- econdata

train <- data[1:58, ]
tune <- data[58:65, ]
test <- data[66:75, ]
#xreg <- as.matrix(xregdata[-75, ])
pselect <- VARselect(econdata)

model <- VAR(train, p = 4)
```
# Evaluate Performance of the Model
```{r}
library(Metrics)
tune_try <- predict(model, n.ahead = length(tune), newdata = tune)

y <- as.data.frame(tune[(nrow(tune) - 7):nrow(tune),])


rmse(tune_try$fcst$Recession, y$Recession)
mae(tune_try$fcst$Recession, y$Recession)
```

# Forecast GDP
```{r}
gdpforecast <- predict(model, n.ahead = 1, newdata = tail(test, 4))$fcst
#gdpforecast <- as.data.frame(gdpforecast)

print(gdpforecast[10])
```

# Augmented Dickey-Fuller Test
```{r, warning=FALSE}
x <- ts(econdata)
adf.test(econdata$Recession)
```

